import { FETCH_ADDITIONAL_INFO, FETCH_ITEM_DETAILS, FETCH_RELATED_ITEMS, FETCH_SPECIFICATION, FETCH_VARIANTS, SET_ADDITIONAL_INFO, SET_DEFAULT_ITEM, SET_ITEM_DETAILS, SET_RELATED_ITEMS, SET_SPECIFICATION, SET_VARIANTS, SET_VARIANT_IMAGES } from "../constants/actionTypes";
...
const itemDetailsReducer = (state = initialState, action) => {
    ...
    switch (action.type) {
        ...
        case SET_ITEM_DETAILS: {
            ...
            const imageskey = Object.keys(data).filter(key => key.includes('img') ? data[key] : "")
            const images = imageskey.map(key => data[key])
            return {
                ...state, data, images, loading: false, defaultVariantItem
            }
        }
