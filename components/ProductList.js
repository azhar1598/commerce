...
import {
  getCategoricalItemCount,
  getCategoricalItemCountAction,
  getCategoricalItemsAction,
  getCategorycalItemsAction,
  getInitialItemsAction,
  getSearchItemsAction,
  getWishlistDetails,
  getWishlistItems,
  searchItems,
} from "../actions";
...
export const ProductList = ({
  ...
  stateStoreDetails,
  stateWishlistItems,
  dispatchCategoricalItems,
  dispatchCategoricalItemCount,
  dispatchSearchItems,
  dispatchInitialItems,
  storeSettings,
  searchItemLocal,
}) => {
  ...
  useEffect(() => {
    if (customerId) {
      const payload = {
        ...
        storeId: stateStoreDetails.store_id,
        page: 1,
        wishlist,
        setWishlist,
        setPage,
  ...
  useEffect(() => {
    const getSearch = async () => {
      ...
      if (
        searchedItem.data != "" &&
        searchedItem.data != undefined &&
        searchedItem.length != 0
      ) {
        const payload = {
          storeId: "storeId",
          searchedItem: searchedItem.data,
          page: 1,
          customerId,
          items,
          setItems,
  ...
  const getProducts = async () => {
    ...
    if (
      ...
      if (
        Object.keys(filterAndSortPayload).length != 0 ||
        (filterAndSortPayload?.filter_groups
          ? Object.keys(filterAndSortPayload.filter_groups).length != 0
          : "" || filterAndSortPayload?.priceRange
          ? Object.keys(filterAndSortPayload.priceRange).length != 0
          : "")
      ) {
        const payload = {
          storeId: stateStoreDetails.store_id,
    ...
    <>
      {!loading && items?.length == 0 ? (
        <div className="flex flex-col mt-24 lg:ml-80">
          <div className="flex">
            <p className="hidden w- lg:flex lg:-mt-12 px-6 text-lg font-montSemiBold">
              {Object.keys(data).length != 0 && data.constructor === Object
                ? data?.category_id != "All Items"
                  ? data.sub_category_name != "undefined"
          ...
          <div className="flex flex-col  lg:-mt-40 mt-16 items-center justify-center w-full lg:h-screen">
            ...
            <p
              className="text-lg font-montSemiBold lg:pt-44"
              style={{
                color: `${
                  storeSettings?.data?.primary_color
                    ? storeSettings?.data?.secondary_color
        ...
        <>
          {loading ? (
            ...
            <div className="flex flex-col  border-t-2 border-slate-200 shadow lg:border-none lg:shadow-none lg:mt-8  lg:ml-80  md:pl-0">
              <div className="flex items-center justify-between lg:mt-   lg:w-full">
                <p className="hidden lg:flex  px-10 pt-3 text-lg font-montSemiBold">
                  ...
                  data.constructor === Object ? (
                    ...
                    searchedItem.length != 0 ? (
                    <p className="">
                      <i className="text-[#0000007F] font-montMedium ">
                        Showing results for
                      </i>{" "}
                      <span
                        style={{
                ...
                <div className="hidden  right-4  lg:pl-0 lg:flex  lg:justify-end w-1/2 ">
                  ...
                  <div
                    className={`px-2 h-4 pr-20 cursor-pointer`}
                    onClick={() => {
                      handleLayout("grid");
                    }}
                  >
                      ...
                      return (
                        <>
                          {item.item_status == "AVAILABLE" ? (
                            <>
                              {/* Future reference for JSOn parse */}
                              {/* image={item.primary_img !== '' || null ? item.primary_img : item.defaultVariantItem?.variant_value_1?.variant_value_images != undefined ? JSON.parse(item.defaultVariantItem?.variant_value_1?.variant_value_images).img_url_1 : ''} */}
                              ...
                              <Product
                                ...
                                stateWishlistItems={stateWishlistItems}
                                setNoMore={setNoMore}
                                grid={grid}
                                item={item}
                                isVeg={item.is_veg == "Y" ? true : false}
                              />
                            </>
                          ) : (
                            ""
          ...
          <div className="absolute right-4 lg:w-60 -mt-9 lg:left-4 lg:pl-0 lg:flex lg:mt-0 lg:ml-24 ">
            <SortFilterModal
              filterAndSortPayload={filterAndSortPayload}
              setFilterAndSortPayload={setFilterAndSortPayload}
              sortOrder={sortOrder}
              serSortOrder={setSortOrder}
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    dispatchCategoricalItems: (payload) =>
      dispatch(getCategoricalItemsAction(payload)),
    dispatchCategoricalItemCount: (payload) =>
      dispatch(getCategoricalItemCountAction(payload)),
