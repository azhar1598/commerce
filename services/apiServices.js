...
export const handleGetShopWidgets = (payload) =>
    ...
    callAPI('GET', `stores/get-seo-details&storeId=582`)


export const getStoreSettingsAPI = () =>
    callAPI('GET', `stores/get-store-display-settings&storeId=${582}`)

export const getStoreDisplaySettingsAPI = () =>
    callAPI('GET', `stores/get-store-settings&storeId=${582}`)

export const getStoreDetailsAPI = (storeId) =>
    callAPI('GET', `stores/get-details&storeId=582`)

export const getBannerImages = (storeId) =>
    callAPI(`GET`, `stores/get-banners&storeId=${582}`)

export const getNewArrivals = (storeId) =>
    callNodeAPI(`GET`, `/api/v1/store-widgets/get-new-arrivals-by-store?storeId=582`)


export const getFeaturedProducts = (storeId) =>
    callNodeAPI(`GET`, `/api/v1/store-widgets/get-best-sellers-by-store?storeId=582`)
...
export const getInitialItems = (payload) => {
    ...
    if (payload.filterAndSortPayload) {
        ...
        return callAPI(
            'POST', `catalog/get-items&storeId=${582}&pageNum=${payload.page}${payload.sortOrder != 'false' ? `&sortOrder=${payload.sortOrder}` : ''}&customerId=${payload.customerId}`,
        ...
        return callAPI(
            'GET', `catalog/get-items&storeId=${582}&pageNum=${payload.page}&customerId=${payload.customerId}`,
        );
    }
}
...
export const getCategories = (storeId) =>
    callAPI(
        'GET', `catalog/get-categories&storeId=${582}`,
    );
...
console.log('filterpayloadAnd payload sort',payload)
    ...
    return callAPI(
        'GET', `catalog/get-items&storeId=${582}&categoryId=${payload.categoryId}&${payload.subCategoryId != 'undefined' && payload.subCategoryId != undefined ? `subCategoryId=${payload.subCategoryId}` : ''}&pageNum=${payload.page}${payload.sortOrder != 'false' || undefined ? `&sortOrder=${payload.sortOrder}` : ''}&customerId=${payload.customerId}`,
    );
}

export const getCategoryItemCount = (storeId, id, subCategoryId) =>
    callAPI('GET', `catalog/get-item-count&storeId=${582}&categoryId=${id}&subCategoryId=${subCategoryId}`)

export const customerSignUp = ({ payload }) =>

    callNodeAPI('POST', `api/v1/customer/register`, {
        storeId: 582, fullName: payload.inputSignUp?.name, password: payload.inputSignUp.password, confirmPassword: payload.inputSignUp.confirm_password, verificationType: payload.method, emailId: payload.method == "EMAIL" ? payload.inputSignUp?.method : '', deviceId: payload.deviceId, phone: payload.method == "EMAIL" ? '' : payload.inputSignUp?.method, isdCode: '91'
    })
...
export const customerLogIn = ({ payload }) => {
    ...
    return callNodeAPI('POST', `api/v1/customer/login`, {
        storeId: 582, password: payload.inputSignUp.password, verificationType: payload.method, emailId: payload.method == "EMAIL" ? payload.inputSignUp?.method : '', phone: payload.method == "EMAIL" ? '' : payload.inputSignUp?.method, isdCode: '91', deviceId: payload.deviceId
    })
}
...
export const verifyOtpAPI = (storeId, customerId, otpCode, verificationType) =>
    callNodeAPI('POST', `api/v1/customer/verify-account`, { storeId: 582, customerId, otpCode, verificationType })
...
export const getSearchItems = (payload) =>
    callAPI('GET', `catalog-search/search-items&storeId=${582}&searchKey=${payload.searchedItem}&pageNum=${payload?.page}&customerId=${payload.customerId}&${payload.sortOrder != 'false' || undefined ? `&sortOrder=${payload.sortOrder}` : ''}`)


export const getOrdersInfo = (customerId) =>
    callAPI('GET', `my-orders/get-orders-by-customer-id&customerId=${customerId}&storeId=${582}`)
...
export const getWalletAmount = (payload, storeId) => {
    ...
    return callAPI('GET', `customer/get-customer-wallet-details&customerId=${payload.customerId}&storeId=582`)
}
export const getWalletTransactions = (payload) =>
    callAPI('GET', `customer/get-wallet-transactions&customerId=${payload.customerId}&storeId=582`)
...
export const addToWishlist = (storeId, customerId, itemId) =>
    callAPI('POST',
        `customer/add-item-to-wishlist&customerId=${customerId}&storeId=${582}&itemId=${itemId}`
    );
...
export const filterApi = (payload) =>
    callAPI('GET', `catalog-search/get-filter-groups&storeId=${582}`)
