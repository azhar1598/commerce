...
export const handlePoliciesAPI = (payload) =>
    callNodeAPI("GET", `/api/v1/store/get-policies?storeId=${STORE_ID}`);
...
export const getNewArrivals = (storeId) =>
    callNodeAPI(
        `GET`,
        `/api/v1/store-widgets/get-new-arrivals-by-store?storeId=${STORE_ID}`
    );

export const getFeaturedProducts = (storeId) =>
    callNodeAPI(
        `GET`,
        `/api/v1/store-widgets/get-best-sellers-by-store?storeId=${STORE_ID}`
    );
...
export const getInitialItems = (payload) => {
    ...
    if (payload.filterAndSortPayload) {
        return callAPI(
            "POST",
            `catalog/get-items&storeId=${STORE_ID}&pageNum=${payload.page}${payload.sortOrder != "false" ? `&sortOrder=${payload.sortOrder}` : ""
        ...
        return callAPI(
            "GET",
            `catalog/get-items&storeId=${STORE_ID}&pageNum=${payload.page}&customerId=${payload.customerId}`
        );
    }
};

export const getCategories = (storeId) =>
    callAPI("GET", `catalog/get-categories&storeId=${STORE_ID}`);
...
export const getCategoricalItems = (payload) => {
    ...
    return callAPI(
        "GET",
        `catalog/get-items&storeId=${STORE_ID}&categoryId=${payload.categoryId}&${payload.subCategoryId != "undefined" && payload.subCategoryId != undefined
            ? `subCategoryId=${payload.subCategoryId}`
            : ""
        }&pageNum=${payload.page}${payload.sortOrder != "false" || undefined
...
export const getCategoryItemCount = (storeId, id, subCategoryId) =>
    callAPI(
        "GET",
        `catalog/get-item-count&storeId=${STORE_ID}&categoryId=${id}&subCategoryId=${subCategoryId}`
    );

export const customerSignUp = ({ payload }) =>
    callNodeAPI("POST", `api/v1/customer/register`, {
        storeId: STORE_ID,
        fullName: payload.inputSignUp?.name,
        password: payload.inputSignUp.password,
        confirmPassword: payload.inputSignUp.confirm_password,
        verificationType: payload.method,
        emailId: payload.method == "EMAIL" ? payload.inputSignUp?.method : "",
        deviceId: payload.deviceId,
        phone: payload.method == "EMAIL" ? "" : payload.inputSignUp?.method,
        isdCode: "91",
    });
...
export const verifyForgotOtp = (payload) => {
    ...
    return callNodeAPI("POST", `/api/v1/customer/verify-otp`, {
        otpCode: payload.OTP,
        customerId: payload.customerId,
        verificationType: payload.method,
    });
...
export const resetPasswordAPI = (input, customerId) =>
    callNodeAPI("POST", `/api/v1/customer/reset-password`, {
...
export const verifyOtpAPI = (storeId, customerId, otpCode, verificationType) =>
    callNodeAPI("POST", `api/v1/customer/verify-account`, {
        storeId: STORE_ID,
        customerId,
        otpCode,
        verificationType,
...
export const getItemDetails = (itemId, customerId) =>
    callAPI(
        "GET",
        `catalog/get-item-details&itemId=${itemId}&customerId=${customerId}`
    );
...
export const getSimilarItems = (itemId) =>
    callNodeProdAPI(
        "GET",
        `/api/v1/widgets/get-related-items?item_id=${itemId}
        `
    );
...
export const getVariantByItemId = (itemId, variantValueId) =>
    callAPI(
        "POST",
        `catalog/get-variant-items-by-item-id&itemId=${itemId}`,
        variantValueId
    );
...
export const getSearchItems = (payload) =>
    callAPI(
        "GET",
        `catalog-search/search-items&storeId=${STORE_ID}&searchKey=${payload.searchedItem
        }&pageNum=${payload?.page}&customerId=${payload.customerId}&${payload.sortOrder != "false" || undefined
            ? `&sortOrder=${payload.sortOrder}`
            : ""
        }`
    );
...
export const getOrdersInfo = (customerId) =>
    callAPI(
        "GET",
        `my-orders/get-orders-by-customer-id&customerId=${customerId}&storeId=${STORE_ID}`
    );
...
export const getWalletAmount = (payload, storeId) => {
    ...
    return callAPI(
        "GET",
        `customer/get-customer-wallet-details&customerId=${payload.customerId}&storeId=${STORE_ID}`
    );
};
export const getWalletTransactions = (payload) =>
    callAPI(
        "GET",
        `customer/get-wallet-transactions&customerId=${payload.customerId}&storeId=${STORE_ID}`
    );
...
export const getCartDetails = (payload) => {
    if (Object.keys(payload.data).length !== 0) {
        if (payload.purchaseId == undefined) {
            return callAPI(
                "POST",
                `orders/add-items-to-cart&customerId=${payload.customerId}&groupId=1`,
                payload.data
            );
        } else {
            return callAPI(
                "POST",
                `orders/add-items-to-cart&customerId=${payload.customerId}&groupId=1&purchaseId=${payload.purchaseId}`,
                payload.data
            );
        }
    }
...
export const addAddress = (customerId, addressData) =>
    callAPI("POST", `customer/add-address&customerId=${customerId}`, {
        customerAddressDetails: addressData,
    });

export const removeAddress = (customerId, addressId) =>
    callAPI(
        "GET",
        `customer/remove-address&customerId=${customerId}&addressId=${addressId}`
    );

export const editAddressAPI = (customerId, addressId, addressData) =>
    callAPI(
        "POST",
        `customer/update-address&addressId=${addressId}&customerId=${customerId}`,
        { customerAddressDetails: addressData }
    );
...
export const setDeliveryAddress = (purchaseId, addressId) => {
    if (addressId) {
        return callAPI(
            "GET",
            `orders/set-delivery-address-id&purchaseId=${purchaseId}&deliveryAddressId=${addressId}`
        );
    } else {
        return null;
    }
...
export const convenienceFlag = (purchaseId, flag) =>
    callAPI(
        "GET",
        `orders/set-convenience-flag&purchaseId=${purchaseId}&flagStatus=${flag}`
    );
...
export const couponApply = (payload) =>
    callAPI(
        "GET",
        `orders/validate-coupon&storeId=${payload.storeId}&couponCode=${payload.coupon}&orderId=${payload.orderId}&customerId=${payload.customerId}`
    );
...
export const initiateCashOnDelivery = (purchase_id, customerId, amount) => {
    return callAPI(
        "POST",
        `orders/confirm-order-payments&purchaseId=${purchase_id}&method=COD`,
        { customerId, amount }
    );
};
...
export const initiateRazorPayOrder = (
    ...
    return callAPI(
        "GET",
        `orders/create-new-rzp-order&purchaseId=${purchaseId}&totalPurchaseAmount=${totalPurchaseAmount}&currency=${currency}`
    );
};

export const razorPayOrderSuccess = (
    purchaseId,
    customerId,
    amount,
    wallet,
    ...
    return callAPI(
        "POST",
        `orders/confirm-order-payments&purchaseId=${purchaseId}&method=${method}`,
        { customerId, amount, useWalletAmount: wallet, id }
    );
};
export const getWishlistAPI = (payload) =>
    callAPI(
        "GET",
        `customer/get-wishlist-items&customerId=${payload?.customerId
        }&pageNum=${1}&storeId=${payload?.storeId}
`
    );
...
export const deleteFromWishlist = (primaryKey) =>
    callAPI(
        "GET",
        `customer/delete-from-wishlist&wishlistId=${primaryKey}
`
    );
...
export const cancelOrder = (orderId, payload) => {
    return callAPI("POST", `my-orders/cancel-order&orderId=${orderId}`, payload);
};
...
export const getAddonsAPI = (payload) =>
    ...
    export const editAddonsAPI = (payload) =>
    callAPI(
        "GET",
        `order/set-add-on-order-item&orderItemId=12345` 
